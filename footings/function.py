...
def func_annotation_valid(function):
    ...
    for k, v in function.__annotations__.items():
        if k != 'return':
            if (v is not Column and 
                   v is not Frame and 
                       v is not Setting and 
                           isinstance(v, type)):
                unk_types.append(k)
            if (isinstance(v, Column) == False and 
                   isinstance(v, Frame) == False and 
                       isinstance(v, Setting) == False):
                not_inste.append(k)
    ...
    assert (len(c) > 0 and len(f) > 0) == False, \
        "Column and Frame types cannot be used together as annotation inputs."
