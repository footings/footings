...
from footings.core.ffunction import TableIn, TableOut, ffunction
...
def test_build_model_tables1_params1():
    ...
    assert_frame_equal(model(table_ab=df, param_divide_by=2).compute(), test)
...
def test_build_model_tables1_params2():
    ...
    test = df.assign(add=(df.a + df.b), subtract=(df.a - df.b))
    assert_frame_equal(
        model(table_ab=df, param_divide_by=2, param_apply_divide=False).compute(), test
    )
