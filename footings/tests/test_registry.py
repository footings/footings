...
class TestRegistry:
    ...
    def test_registry_using_columns(self):
        ...
        assert all(i in ['v', 'i', 'disc_factor', 'disc_cash', 'cash', 'pv'] for i in reg1.list_registered())
        assert all(i in ['cash', 'disc_factor'] for i in reg1.predecesors('disc_cash')) 
        assert all(i in ['cash', 'i', 'v', 'disc_factor'] for i in reg1.ancestors('disc_cash'))
        ...
        assert all(i in ['v', 'i', 'disc_factor', 'disc_cash', 'cash', 'pv'] for i in reg2.list_registered())
